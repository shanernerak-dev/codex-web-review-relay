...
Effective round: `3`
Target PR: `#1`
Review scope: 公开仓库 README 文档质量评审（第三轮，trigger envelope 已嵌入评审指令）
...
本轮变更：
- GitHub PR comment 从"必须"改为"可选"，新增两层依赖模型
- 更新了双向信息传递描述（relay MCP 回传为主路径，PR comment 为可选正式记录）
- trigger envelope 已嵌入评审执行指令，无需读取 handoff 文件
- content script stability window 从 1.5s 增大到 30s（需扩展重新加载后生效）
...
- `src/envelope.ts` — trigger envelope 生成（6 个动态字段 + 2 条固定指令）
