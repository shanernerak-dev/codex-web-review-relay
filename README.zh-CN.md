...
Reviewer 可见的 envelope 按以下顺序和大小写逐字冻结：
...
安装器会将通用 exporter 复制到本机用户级安装目录，并在配置中记录绝对 `exporterPath`。它不再绑定某个仓库；每次请求根据绝对 handoff 文件解析仓库。
...
Native host 以 `python <exporterPath> relay-export <repository-relative-handoff-path>` 形式调用 exporter，并将解析出的仓库设为 `cwd`。Exporter 必须：
...
```json
{
  ...
  "exporterPath": "<本机用户级安装目录>/relay_export_helper.py",
...
关键字段：
- `exporterPath`：本机用户级安装目录中的 relay-owned exporter；realpath 必须严格位于 trusted install root 内。
