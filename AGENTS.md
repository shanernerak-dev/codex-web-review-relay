...
1. relay 是 localhost-only transport；在 Stage 3 relay-only contract 完成并验收前，Stage 1/Stage 2 均沿用 v1 PR-comment mode：目标 PR comment 是 formal verdict 来源，relay `assistant_output` 只作为 transport evidence。当前 Stage 3 acceptance review 可由 Maintainer 明确授权 commit-only pilot 使用完整 `assistant_output` 作为本轮验收的 formal source；这不是对外通用可用性的声明，只有 Stage 3 acceptance 后该能力才成为一般契约。
