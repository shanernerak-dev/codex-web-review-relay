...
- Stage 3 round-07（`stage3-main/round-07-review-fix`）：implementation commit `6990ec4a3af9d7746164d250977bfac0fd8998ac`，reviewed head `9c53ce969bcb05880b757a212be87fbf48fe165f`。Review response 已存在并返回 `REQUEST CHANGES`，新增 `RGEN-S3-011` 至 `RGEN-S3-014`；transport 结果为 `TIMEOUT / TURN_DEADLINE_EXCEEDED`。Diagnostics 证明事件序列止于 `trigger_received → monitor_started → trigger_accepted`，从未出现 `user_turn_observed`，formal source 为 Maintainer transfer。当前修复不得预写 Stage 3 acceptance。
- Stage 3 round-08（`stage3-main/round-08-review-fix`）：reviewed head `36bbfff58fd2ad57a2d4e53f42b2ea7c25a18e93` 未进入 Web review。Job `5af991b6-4f4f-480c-a40a-c14800de7425` 在 dispatch receipt gate 返回 `SEND_UNCERTAIN / NATIVE_DISPATCH_WRITE_FAILED`；diagnostics 记录 `baseline_count=0`、`candidate_count=2`、`exact_match_count=0` 与 `SEND_CLICK_RECEIPT_MISSING`，确定失败是“已出现两个新增 user DOM candidate，但逐节点均不等于完整 envelope”。该轮没有 reviewer verdict；后续修复只组合具有同一 stable turn identity 的 user fragments，再比较 canonical text，避免把“两个 candidate”未经 identity 证据直接等同为一个 turn。
